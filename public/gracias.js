...
async function verify(){try{
 ...
 if(data.paid){confirmed=data;const cfg=await configuration;$('state-icon').textContent='✓';$('state-title').textContent='¡Tu pago está confirmado!';$('state-copy').textContent='¡Ya tienes tu lugar! Entra al grupo de WhatsApp: por allí enviaremos el enlace de acceso a la clase y los recordatorios del taller. Guarda también tu referencia de compra.';$('whatsapp-action').href=cfg.whatsappUrl||'https://chat.whatsapp.com/B8WmTjXB53w6yMZji9maiE';$('whatsapp-action').hidden=false;$('state-action').hidden=true;await purchase();}
