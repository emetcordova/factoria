import {track,consentAllowed} from './app.js';
...
async function verify(){try{
 ...
 if(data.paid){confirmed=data;$('state-icon').textContent='✓';$('state-title').textContent='¡Tu pago está confirmado!';$('state-copy').textContent='Ya diste el primer paso. Guarda tu referencia y escríbenos a @emetcordova para coordinar tu acceso al taller, al aula virtual y a la comunidad.';$('state-action').href='https://www.instagram.com/emetcordova/';$('state-action').textContent='Coordinar mi acceso ↗';$('state-action').hidden=false;await purchase();}
